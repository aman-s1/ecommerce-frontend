...
const Shop: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (token) {
                    const userResponse = await fetch('https://ecommerce-backend-aman-s1s-projects.vercel.app/shop', {
                        method: 'GET',
                ...
                // Fetch all products
                const productsResponse = await fetch('https://ecommerce-backend-aman-s1s-projects.vercel.app/shop/products', {
                    method: 'GET',
                    headers: { 'Authorization': `Bearer ${token || ''}` },
                });
                ...
                // Fetch cart items
                const cartResponse = await fetch('https://ecommerce-backend-aman-s1s-projects.vercel.app/cart/get-items', {
                    method: 'GET',
                    headers: { 'Authorization': `Bearer ${token}` },
                });
                if (cartResponse.ok) {
    ...
    const handleAddItem = async (name: string, description: string, price: number, image: string) => {
        try {
            ...
            const response = await fetch('https://ecommerce-backend-aman-s1s-projects.vercel.app/shop/add-product', {
                method: 'POST',
                headers: {
    ...
    const handleRemoveItem = async (id: string) => {
        try {
            ...
            const response = await fetch(`https://ecommerce-backend-aman-s1s-projects.vercel.app/shop/products/${id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` },
            });
